...
from openmedic.core.shared.services.objects.monitor import MonitorOpBase, MonitorOpError
...
@dataclass
class CheckPoint(MonitorOpBase):
    ...
    def initialize(cls, 
        ...
        **kwargs
        ...
        if not model_path:
            raise MonitorOpError("`model_path` can not found in config.")

        if not model_path.endswith(".pth"):
            raise MonitorOpError("Can not save model with different format `.pth`")

        if os.path.exists(model_path) and not is_replace:
            raise MonitorOpError(f"The {model_path} is already exist! If you want to replace that file, please set `is_replace` is true")

        if save_best:
            patience: Optional[int] = save_best.get("patience", None)
            if patience == 0:
                raise MonitorOpError("Can not set `patience` equal to 0. If you do not want apply earl stopping, please remove `patience`.")
